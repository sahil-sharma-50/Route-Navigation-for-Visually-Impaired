        ...
        if filename.endswith(".jpg") or filename.endswith(".png"):
            ...
            if image is not None:
                ...
                binary_mask = cv2.resize(
                    binary_mask, (512, 256), interpolation=cv2.INTER_NEAREST
                )
...
if __name__ == "__main__":
    if len(sys.argv) != 7:
        print(
            "Usage:# python inference.py path/to/model path/to/mapillaryImages path/to/mapillaryLabels "
    ...
    with torch.no_grad():
        ...
        for image_file in tqdm(test_image_files, position=0, leave=True):
            ...
            seg_output_image.save(
                os.path.join(model_output_folder, f"seg_mask_{image_file}")
            )
            bin_output_image.save(
                os.path.join(
                    prediction_binary_dir, f"{image_file.replace('.jpg', '.png')}"
                ),
                format="PNG",
            )
