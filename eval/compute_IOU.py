...
def compute_metrics(prediction_dir, ground_truth_dir):
    ...
    for common_file in common_files:
        ...
        if prediction_mask.shape != ground_truth_mask.shape:
            print(
                f"Skipping {common_file} due to shape mismatch. Prediction shape: {prediction_mask.shape}, Ground truth shape: {ground_truth_mask.shape}"
            )
            continue
...
if __name__ == "__main__":
    ...
    with open("model-IOU.txt", "w") as file:
        ...
        try:
            sys.stdout = file
            # Base metrics
            average_iou, average_precision, average_f1 = compute_metrics(
                base_prediction_dir, ground_truth_dir
            )
            ...
            # Fine-tuned metrics
            average_iou, average_precision, average_f1 = compute_metrics(
                tuned_prediction_dir, ground_truth_dir
            )
